...
public class VariableServerConnection {
		...
		try {
			...
			while (true) {
				...
				if (packetId == 0) {
					...
					for (Listener listener:listeners){try{listener.receivedMessage(server, message);}catch(Exception e){}}
				}
			}
		} catch (Exception e) {
			//e.printStackTrace();
	...
	private void sendVariable(String variable, String value) {
		try {
			...
			PlugCubeBuildersIn.getInstance().reportProblem("Could not send variable", e);
		}
	}
	...
	private void sendThread() {
		try {
			...
			PlugCubeBuildersIn.getInstance().reportProblem("VariableServer send thread crashed", e);
			tryClose(socket);
		}
	}
